 ...
 ****************************************************/
function getLogoSrc() {
  // 1) Local file (best for Vercel static)
  if (typeof SCHOOL_LOGO_URL === "string" && SCHOOL_LOGO_URL.trim()) {
    return SCHOOL_LOGO_URL.trim();
  }
...
function createCard(t, config) {
  ...
  if (currentMode === "front") {
    ...
    div
      .querySelector('[data-action="print-back"]')
...
function printSingleCard(t, side) {
  ...
  w.document.write(
    `<html><head><title>Print Card</title>${css}</head><body>${htmlContent}</body></html>`
  );
  w.document.close();
}
